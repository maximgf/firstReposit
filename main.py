...
import json
from exceptions import operation_exception
...
def get_rests():
    ...
    if 'start_period' not in args.keys() or 'stop_period' not in args.keys():
        raise operation_exception("Date None")
...
def get_nomens_rests(nomenculature_id):
    ...
    if not nomens:
        raise operation_exception("Date is not valid")
...
def debits_recipe(recipe_id):
    ...
    if 'storage_id' not in args:
        raise operation_exception("Conteiner None")
    ...
    if not recipes or not storages_:
        raise operation_exception("Data is not valid!")
